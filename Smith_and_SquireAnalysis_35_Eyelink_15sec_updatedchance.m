...
%xlfile =
%'AllSubjects_15sec_matlab_input_data_ver2_rounded_xy_coordinates.xlsx';
%%Use the above line for running all subject data and the below line for
%%running new subject data
xlfile = 'New_Subject_15sec_matlab_input_data_ver2_rounded_xy_coordinates.xlsx';
...
for subject = 3:length(sheets) %1st and 2nd sheets are data summary
    ...
    [~,~,raw] = xlsread(xlfile,subject,'A1:AC5000');
...
save('SmithSquireData_35_processed','subjectdata') %save data since takes so long to run
...
tic
load('SmithSquireData_35_processed.mat');
...
%             
        ...
        else %if not enough fixations keep structure by placing NaNs
    ...
    count = 0; %index since some pairs might not have enough fixations
     while total < 100 && count < size(Chance_row,1)  %chance row is for the random image pairs
        count = count + 1;
...
%             end
            ...
            for i = 1:size(novelmap,2)
                if all(all(novelmap{i} == mean(mean(novelmap{i})))); %if no fixations aka flat PDF
                    Chanceoverlap{subject}(total+1,i) = 0;
                else
...
save('KLDivergenceData_35_processed','overlap','Chanceoverlap','trialdifference','imagepairs')
...
filename = 'New_Subject_KL_Data_15sec_processed_chance_smith.xls';
load('SmithSquireData_35_processed.mat')
load('KLDivergenceData_35_processed.mat')
