...
xlfile = '5_REMOVETRIALS_Old_New_Experiment_NoMemory_2Halves_ViewThenMemory_Strong_RAW_TEMP.xlsm';
...
for subject = 3:length(sheets) %1st and 2nd sheets are data summary
    ...
    [~,~,raw] = xlsread(xlfile,subject,'A1:AC1000');
...
save('SmithSquireData','subjectdata') %save data since takes so long to ru
...
for subject = 1:length(subjectdata)
    ...
    for pairs = 1:size(imagepair,2);
        ...
        novelmap = cell(1,3); %Fixation matrix for a study phase image
        familiarmap = cell(1,3); %Fixation matrix for the same image but during test phase image
        ...
        if size(familiarfixations,2) >= 5 && size(novelfixations,2) >= 5
            ...
            for fix = 1:maxfixes
                ...
                novelmap{3}(fixy,fixx) = novelmap{3}(fixy,fixx) + 1; % all fixations
            end
            ...
            for fix = 1:maxfixes;
                ...
                familiarmap{3}(fixy,fixx) = familiarmap{3}(fixy,fixx) + 1; % all fixations
            end
    ...
    while total < 100 %want 100 pairs to calculate chance levels
        ...
        if size(familiarfixations,2) >= 5 && size(novelfixations,2) >= 5
            ...
            novelmap = cell(1,3);  %Fixation matrix for a study phase image 1
            familiarmap = cell(1,3);  %Fixation matrix for a study phase image 2
            for i = 1:size(novelmap,2);
                novelmap{i} = zeros(imageY,imageX);
                familiarmap{i} = zeros(imageY,imageX);
            end
...
save('KLDivergenceData','overlap','chanceoverlap','trialdifference','imagepairs')
%save raw data since takes an extremely long time to run
%overlap/chance overlap column format: fixations 1-5, fixations 6-10, all fixations
toc
...
filename = 'Subject_and_KL_Data.xls';
load('SmithSquireData.mat')
load('KLDivergenceData.mat')
...
for subject = 1:length(subjectdata);
    ...
    raw = {'Study Trial','Image','Adjusted X','Adjusted Y','Test Trial','Image',...
        ...
        'KL Divergence 6-10','KL Divergence All fixations','Chance KL 1-5',...
        'Chance KL 6-10','Chance all fixations'}; %1st line, title line
    ...
    for trial = 1:size(overlap{subject},1);
        ...
        raw{trial+1,13} = overlap{subject}(trial,3); %KL divergence all fixations;
    end
    
    % write chance KL divergence data to raw cell array
    for trial = 1:size(chanceoverlap{subject});
        raw{trial+1,14} = chanceoverlap{subject}(trial,1); %KL divergence fixations 1-5
        raw{trial+1,15} = chanceoverlap{subject}(trial,2); %KL divergence fixations 6-10
        raw{trial+1,16} = chanceoverlap{subject}(trial,3); %KL divergence all fixations;
    end
